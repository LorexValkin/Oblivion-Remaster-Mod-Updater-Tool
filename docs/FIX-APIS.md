...
- `package-store-decoder-placeholder-repair-v1` repairs one decoder-only `/Engine/UnknownPackage` placeholder when the source package-store graph identifies exactly one absent resolved identity and the dependency exposes exactly one package-named top-level public export. The rebuilt package must restore the original package-store import set.
