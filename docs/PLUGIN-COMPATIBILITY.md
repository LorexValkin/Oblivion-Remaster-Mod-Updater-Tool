...
| Composite Unreal-container replacements across asset domains | Decoded export classes, exact package identity, current-template topology, serialized reference roles, dependency closure, and rebuilt roundtrip evidence | Supported only when every package passes `native-composite-package-rebase-v1`; ambiguous additive Blueprints, missing authored packages, and unsupported classes remain report-only |
...
- `native-composite-package-rebase-v1` applies the same fail-closed rebuild boundary across decoded StaticMesh, Texture2D, material-instance, SkeletalMesh, and exact current-template packages. Current import tables are reused only under identity/topology equivalence, while skeletal references require serialized role evidence and authored exports/sidecars remain preserved.
