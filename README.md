...
- Supports guarded homogeneous Unreal-container paths, heterogeneous existing-game StaticMesh/Texture2D replacements, and a system-wide composite rebase path for decoded StaticMesh, Texture2D, material-instance, SkeletalMesh, and exact current-template packages. Package identity, serialized reference roles, dependency closure, authored imports, and payload preservation are verified through rebuild and round trip.
