...
Version 0.5.3 adds guarded support for custom-project StaticMesh mods and fixes a crash caused by legacy cooked `BodySetup` physics data. A completed conversion means the files passed the tool's structural checks; you still need to test the result in-game.
...
- Resolves supported custom mesh dependencies against the current game.
...
### Why does a custom mesh report say collision was removed?

Some older meshes contain cooked physics bytes that the current game cannot load. The updater removes that incompatible cooked section while preserving the serialized collision properties used to rebuild it. This prevents the known `BodySetup` loader crash, but you still need to check collision in-game.
...
Download the `OBR-Mod-Updater-v0.5.3-windows-x64.zip` release file, extract it, and start with `README.txt` inside the ZIP.
