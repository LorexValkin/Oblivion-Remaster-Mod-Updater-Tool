...
- `native-composite-package-rebase-v2` applies the same fail-closed rebuild boundary across decoded meshes, textures, material instances, generated Blueprints, `/Script/Altar` TES forms, and exact current-template packages. Missing package names require raw NameMap/package-ID proof, identity aliases require serialized Blueprint role proof, decoder placeholders require a unique package-store delta and public export, and authored exports/sidecars remain preserved.
